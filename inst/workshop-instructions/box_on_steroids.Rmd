...
5. Let's consider the following input binding skeleton. `{charpente}` provide tools to create input binding boilerplate, through the `create_input_binding`. Apply it to create the `boxBinding.js` script. Once created, replace all `...` by the appropriate methods and names.
...
6. Create an HTML dependency pointing to a script `boxBinding.js`. This will be necessary to test the developed code.
...
8. Implement the `getValue` method. **Hint**: `hasClass` allows to check whether the selected elements has the given class. `getValue` **must** call `return` at the end of its definition.

9. It's time to create the R component. The `box2` function is alread available in the `./R` folder. Add it the *id* parameter necessary to link the HTML element to the JS binding. Once done add the JS dependency to the `box2` element. This will allow the component to be standalone (it includes its own dependencies).
...
Why isn't the `updateBox2` doing anything?
